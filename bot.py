...
class OKXAdaptiveGridBot:
    ...
    'options': {
        ...
        # ---------------------------
        self.grid_count = 10
        self.grid_percent_range = 0.06   # 6% total range around price
        self.recenter_threshold = 0.03   # recenter if price moves 3% out
    ...
    def check_recenter(self, current_price):
        ...
        if drift > self.recenter_threshold:
            print("\n🔄 RECENTERING GRID (price drift detected)")
            self.generate_grid(current_price)

            # reset orders (important)
