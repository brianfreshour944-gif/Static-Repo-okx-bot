...
def get_daily_loss_today() -> float:
    """
    Calculate today's net P&L by summing trade values from the trades table
    filtered to DATE(timestamp) = today.  Buys are negative (spend),
    sells are positive (receive).  Returns a negative number when losing.
    This approach is immune to stale running totals from previous days.
    """
    try:
        with engine.connect() as conn:
...
class ReactiveGridBot:
    ...
    async def safety_monitor(self):
        """
        Checks today's realized P&L (from trades table, today only).
        Immune to stale daily_loss values from previous sessions.
        """
        while self.running:
            ...
            try:
                ...
                if daily_loss_today < 0:
                    logger.info(
                        f"📊 Today's P&L: {daily_loss_today:+.2f} | "
                        f"Limit: -${limit:.2f}")
    ...
    async def run(self):
        ...
        # Check TODAY's loss before doing anything — not all-time loss
        daily_loss_today = get_daily_loss_today()
        if daily_loss_today <= -MAX_DAILY_LOSS_USDT:
            logger.critical(
                f"Today's loss limit already reached "
                f"({daily_loss_today:.2f}). Not starting.")
            return

        logger.info(f"📊 Today's starting P&L: {daily_loss_today:+.2f}")
        ticker = await self.fetch_ticker()
